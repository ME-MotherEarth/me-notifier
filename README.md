# MotherEarth events notifier

The notifier service is a component that receives block events synchronously
from motherearth observer nodes, and it forwards them to a subscribing component
...
these [docs](https://docs.motherearth.com/integrators/observing-squad/) 
cover the whole process. 

The required configs for launching an observer/s with a driver attached,
can be found [here](https://github.com/ME-MotherEarth/me-motherearth/blob/master/cmd/node/config/external.toml).
...
The [config](https://github.com/ME-MotherEarth/me-notifier/blob/main/cmd/notifier/config/config.toml) file:
...
- Protocol based events, such as `MECTTrasnfer` or `NFTCreate`
...
Note: 
- The address field is `bech32` encoded with the tag `moa`.
